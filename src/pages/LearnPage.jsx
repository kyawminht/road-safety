...
export default function LearnPage() {
  ...
  return (
    <div className="flex-1 overflow-hidden flex flex-col bg-black">
      ...
      {/* ── Floating Topic Bar ── */}
      <div className="relative z-20 flex items-center justify-between gap-2 px-5 pt-2 pb-1 pointer-events-none">
      ...
      >
        <div className="md:max-w-6xl md:w-full md:px-8 md:pt-4 md:pb-12 md:grid md:grid-cols-2 md:gap-8 lg:gap-12">
          {allCards.map((card, idx) => (
            <div
              key={card.id}
              ref={(el) => { cardRefs.current[idx] = el; }}
              data-index={idx}
              className="snap-start snap-always h-[calc(100dvh-56px-20px)] md:h-auto md:aspect-[4/5]"
            >
              <RevealCard
                card={card}
                cardIndex={idx}
          ...
          {/* ── Completion Screen ── */}
          <div className="snap-start snap-always h-[calc(100dvh-56px-20px)] md:h-auto md:aspect-[4/5] flex flex-col items-center justify-center bg-gradient-to-b from-teal-900 to-black text-white text-center p-8">
            <motion.span
