import { motion, AnimatePresence } from 'framer-motion';
import { useAuth } from '../hooks/useAuth.jsx';

export default function AuthPrompt({ open, onClose }) {
  const { signInWithGoogle, loading } = useAuth();

  const handleSignIn = async () => {
    const { error } = await signInWithGoogle();
    if (!error) onClose();
  };

  return (
    <AnimatePresence>
      {open && (
        <>
          {/* Backdrop */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="fixed inset-0 z-50 bg-black/50"
          />

          {/* Bottom sheet */}
          <motion.div
            initial={{ y: '100%' }}
            animate={{ y: 0 }}
            exit={{ y: '100%' }}
            transition={{ type: 'spring', stiffness: 300, damping: 30 }}
            className="fixed bottom-0 left-0 right-0 z-50 bg-white rounded-t-3xl px-6 pt-6 pb-10
              shadow-[0_-8px_32px_rgba(0,0,0,0.15)]"
            style={{ paddingBottom: 'max(env(safe-area-inset-bottom, 0px) + 24px, 40px)' }}
          >
            {/* Handle */}
            <div className="w-10 h-1 rounded-full bg-gray-300 mx-auto mb-5" />

            {/* Icon */}
            <div className="w-16 h-16 rounded-2xl bg-teal-50 flex items-center justify-center mx-auto mb-4">
              <span className="text-3xl">💾</span>
            </div>

            {/* Text */}
            <h3 className="text-lg font-bold text-gray-900 text-center mb-2">
              တိုးတက်မှုကို သိမ်းဆည်းပါ
            </h3>
            <p className="text-sm text-gray-500 text-center mb-6 max-w-xs mx-auto leading-relaxed">
              Google ဖြင့် ဝင်ရောက်ပါ၊ သင်၏ တိုးတက်မှုများကို အချိန်မရွေး ပြန်လည်ရရှိနိုင်ပါသည်
            </p>

            {/* Google sign-in button */}
            <button
              onClick={handleSignIn}
              disabled={loading}
              className="w-full flex items-center justify-center gap-3 py-3.5 rounded-xl
                border-2 border-gray-200 hover:border-gray-300 bg-white
                text-gray-700 font-semibold text-base transition-colors disabled:opacity-50"
            >
              <svg viewBox="0 0 24 24" className="w-5 h-5" xmlns="http://www.w3.org/2000/svg">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/>
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
              </svg>
              {loading ? 'ဝင်နေသည်...' : 'Google ဖြင့် ဝင်ရန်'}
            </button>

            {/* Skip */}
            <button
              onClick={onClose}
              className="w-full py-3 text-gray-400 text-sm font-medium mt-2 hover:text-gray-600 transition-colors"
            >
              နောက်မှ
            </button>
          </motion.div>
        </>
      )}
    </AnimatePresence>
  );
}
