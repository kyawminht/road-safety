    ...
    <div className="px-5 py-3 border-t border-white/5">
        ...
        {focused && (
          ...
          >
            <p className="text-[11px] text-white/30 mb-2 flex items-center gap-1">
              💬 ကျေးဇူးပြု၍ ကောင်းမွန်စွာ ရေးသားပေးပါ
            </p>
          </motion.div>
        )}
      </AnimatePresence>
          ...
          {text.trim() && (
            <motion.button
              ...
              className="w-10 h-10 rounded-xl bg-teal-500 flex items-center justify-center text-white
                shadow-lg shadow-teal-500/25 shrink-0 disabled:opacity-40 disabled:shadow-none"
            >
      ...
      <AnimatePresence>
        {focused && charCount > 0 && (
          ...
          >
            <span className={`text-[11px] tabular-nums ${isOverLimit ? 'text-red-400' : 'text-white/25'}`}>
              {charCount}/{MAX_CHARS}
            </span>
          </motion.div>
        )}
